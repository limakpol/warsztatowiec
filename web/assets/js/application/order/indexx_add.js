$(document).ready(function()
{
    ...
    /* PRODUCER */
    $('#order_indexx_add_indexx_producer option:first-child').after('<option value="new">-- nowy --</option>');

    $(document).on('change', '#order_indexx_add_indexx_producer', function()
    {
        $('#order_indexx_add_indexx_producer + ul').remove();
        ...
        $.ajax({
            ...
            success: function(data) {
                if(!data['errors'])
                {
                    ...
                    $('#order_indexx_add_indexx_producer option:nth-child(2)').after(option);

                    $('#order_indexx_add_indexx_producer').val(data['producerId']).change();
                }
            }
        });
    });
    ...
    $(document).on('click', '#searchable-good tr:not(:first-child)', function()
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('#order_indexx_add_indexx_good_id').val(data[0].id);
                $('#order_indexx_add_indexx_good_name').val(data[0].name);
                $('#order_indexx_add_indexx_good_measure').val(data[0].measure_id);
                $('#order_indexx_add_indexx_good_quantity').val(data[0].quantity);
                $('#order_indexx_add_indexx_good_remarks').val(data[0].remarks);
                ...
                data[1].forEach(function(category)
                {
                    var input = '<input type="hidden" ' +
                        'name="order_indexx_add[indexx][good][categories][]" ' +
                        'value="' + category[0] + '">';
                ...
                {
                    var input = '<input type="hidden" ' +
                        'name="order_indexx_add[indexx][good][car_models][]" ' +
                        'value="' + carModel[0] + '">';
    ...
    $(document).on('click', '#searchable-indexx tr:not(:first-child)', function()
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('#order_indexx_add_indexx_id').val(data.id);
                $('#order_indexx_add_indexx_name').val(data.name);
                $('#order_indexx_add_indexx_producer').val(data.producer_id);
                $('#order_indexx_add_indexx_quantity').val(data.quantity);
                $('#order_indexx_add_indexx_unit_price_net').val(data.unit_price_net);
                $('#order_indexx_add_unit_price_net').val(data.unit_price_net);
                ...
                var measureShortcut = $('#order_indexx_add_indexx_good_measure option:selected').data('shortcut');

                $('label[for=order_indexx_add_quantity]').text('Ilość [' + measureShortcut + ']');
    ...
    $(document).on('click', '#categories-selectable-modal main ul .addable .btn-add.active', function()
    {
        ...
        var goodId = $('#order_indexx_add_indexx_good_id').val();
...
{
    $("input[name*='order_indexx_add[indexx][good]'] + ul").remove();
    $("select[name*='order_indexx_add[indexx][good]'] + ul").remove();
    $("textarea[name*='order_indexx_add[indexx][good]'] + ul").remove();
...
{
    $("input[name*='order_indexx_add[indexx][good]']").val('').change();
    $("select[name*='order_indexx_add[indexx][good]']").val('').change();
    $("textarea[name*='order_indexx_add[indexx][good]']").val('').change();
...
{
    $('#order_indexx_add_indexx_name + ul').remove();
    $('#order_indexx_add_indexx_producer + ul').remove();
    $('#order_indexx_add_indexx_quantity + ul').remove();
    $('#order_indexx_add_indexx_unit_price_net + ul').remove();
...
{
    $('#order_indexx_add_indexx_name').val('').change();
    $('#order_indexx_add_indexx_producer').val('').change();
    $('#order_indexx_add_indexx_quantity').val('').change();
    $('#order_indexx_add_indexx_unit_price_net').val('').change();
